...
-- ============================================================
SET @sql_col = IF((SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'OH_MEDICALDSR' AND COLUMN_NAME = 'MDSR_AFM_ID') = 0,
    'ALTER TABLE OH_MEDICALDSR ADD COLUMN MDSR_AFM_ID INT DEFAULT NULL',
    'SELECT 1');
...
SET @sql_fk = IF((SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'OH_MEDICALDSR' AND CONSTRAINT_NAME = 'FK_MEDICAL_ARTICLE_FAMILY') = 0,
    'ALTER TABLE OH_MEDICALDSR ADD CONSTRAINT FK_MEDICAL_ARTICLE_FAMILY FOREIGN KEY (MDSR_AFM_ID) REFERENCES OH_ARTICLE_FAMILIES(AFM_ID)',
    'SELECT 1');
...
SET @sql_idx = IF((SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'OH_MEDICALDSR' AND INDEX_NAME = 'IDX_MEDICALDSR_AFM_ID') = 0,
    'CREATE INDEX IDX_MEDICALDSR_AFM_ID ON OH_MEDICALDSR(MDSR_AFM_ID)',
    'SELECT 1');
