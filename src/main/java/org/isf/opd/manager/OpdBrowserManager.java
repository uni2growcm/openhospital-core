...
public class OpdBrowserManager {
	...
	/**
	 * Returns paged list of OPDs associated to specified patient ID
	 */
	public PagedResponse<Opd> getOpdByPatientIdPageable(int patientcode, int page, int size)
		throws OHServiceException {
		return ioOperations.getOpdListPageable(patientcode, page, size);
	}

	/**
	 * Returns paged list of OPDs with specified Progressive in Year number
	 */
	public PagedResponse<Opd> getOpdByProgYearPageable(int progYear, int page, int size)
		throws OHServiceException {
		return ioOperations.getOpdByProgYearPageable(progYear, page, size);
	}
	/**
	 * Count total number of OPDs matching the given criteria
	 */
	public long countTotalOpds(Ward ward, String diseaseTypeCode, String diseaseCode,
	                           LocalDate dateFrom, LocalDate dateTo, int ageFrom, int ageTo,
	                           char sex, char newPatient, String user) throws OHServiceException {
		List<Opd> allOpds = getOpd(ward, diseaseTypeCode, diseaseCode,
			dateFrom, dateTo, ageFrom, ageTo, sex, newPatient, user);
		return allOpds.size();
	}
}
