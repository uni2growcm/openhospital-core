...
public class LabbookAPIClientFactory {
	...
	public ILabbookService getILabbookService() {
		if (iLabbookService != null) {
			return iLabbookService;
		}

		RestClient restClient = RestClient.builder().baseUrl(baseUrl).requestInterceptor(requestInterceptor).build();
		iLabbookService = HttpServiceProxyFactory
			.builderFor(RestClientAdapter.create(restClient))
			.build()
			.createClient(ILabbookService.class);
