...
public class PartnerBrowserManager {
	...
	/**
	 * Returns the active {@link Partner} with the given id.
	 * @param code the code of the partner to retrieve.
	 * @return the {@link Partner} with the given id.
	 * @throws OHServiceException if no active partner is found with the given id, or the fetch fails.
	 */
	public Partner getPartner(Integer code) throws OHServiceException {
		return ioOperations.getById(code)
			.orElseThrow(() -> new OHServiceException(
				new OHExceptionMessage(MessageBundle.formatMessage("angal.partner.notfound.msg", String.valueOf(code)))));
	}

	/**
	 * Returns the active {@link Partner} with the given code, if it exists.
	 * @param code the code of the partner to retrieve.
	 * @return an {@link java.util.Optional} containing the {@link Partner} if found and active, or empty otherwise.
	 ...
	 * @throws OHServiceException when fails to fetch the partners.
	 */
	public List<Partner> getPartnersByType(PartnerType type) throws OHServiceException {
	...
	/**
	 * Searches for active {@link Partner}s whose name, contact person or type description contains the
	 * given keyword (case-insensitive). Returns all active partners if the keyword is {@code null} or blank.
	 ...
	 * @param code the id of the partner to delete.
	 * @throws OHServiceException when fails to delete the partner.
	 */
	public void deletePartner(int code) throws OHServiceException {
		ioOperations.softDelete(code);
	}

	private void validatePartner(Partner partner) throws OHDataValidationException {
		List<OHExceptionMessage> errors = new ArrayList<>();

		if (partner.getName() == null || partner.getName().trim().isEmpty()) {
			errors.add(new OHExceptionMessage(MessageBundle.getMessage("angal.partner.validation.name.required.msg")));
		}

		if (partner.getType() == null) {
			errors.add(new OHExceptionMessage(MessageBundle.getMessage("angal.partner.validation.type.required.msg")));
		}
